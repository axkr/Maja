...
class EigenvalueDecompositionImpl {
    ...
    private void tql2() {
        ...
        for (int l = 0; l < n; l++) {
            ...
            int m = l;
            while (m < n) {
                if (Math.abs(e[m]) <= Maja.EPSILON * tst1)
                    break;
                m++;
            }
            if (m > l) {
                ...
                do {
                    ...
                    e[l] = s * p;
                    d[l] = c * p;
                } while (Math.abs(e[l]) > Maja.EPSILON * tst1);
            }
            d[l] = d[l] + f;
            e[l] = 0.0;
        }
    ...
    private void hqr2() {
        ...
        while (n >= low) {
            ...
            while (l > low) {
                ...
                if (s == 0.0) s = norm;
                if (Math.abs(H[l][l - 1]) < Maja.EPSILON * s)
                    break;
                l--;
            }
            if (l == n) {
                ...
                while (m >= l) {
                    ...
                    if (Math.abs(H[m][m - 1]) * (Math.abs(q) + Math.abs(r)) <
                            Maja.EPSILON * (Math.abs(p) * (Math.abs(H[m - 1][m - 1]) + Math.abs(z) +
                                    Math.abs(H[m + 1][m + 1])))) break;
                    m--;
                }
                for (int i = m + 2; i <= n; i++) {
                    H[i][i - 2] = 0.0;
                ...
                for (int i = n - 2; i >= 0; i--) {
                    ...
                    if (e[i] < 0.0) {
                        ...
                        if (e[i] == 0) {
                            ...
                            vi = (d[i] - p) * 2.0 * q;
                            if (vr == 0.0 & vi == 0.0)
                                vr = Maja.EPSILON * norm * (Math.abs(w) + Math.abs(q) +
    ...
    public Complex[] getEigenvalues() {
        Complex[] complexEigenvalues = new Complex[n];
        for (int i = 0; i < n; i++)
            complexEigenvalues[i] = new Complex(d[i], e[i]);
