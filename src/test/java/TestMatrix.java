...
public class TestMatrix {
  ...
  public void testBasicToString() {
    Matrix<Complex> m = new Matrix<>(new Complex[][] {{new Complex(1, 2), new Complex(3, 4)},
  ...
  public void testMultilineToString() {
    Matrix<String> m = new Matrix<>(new String[][] {{"Hello", "World"}, {"Foo", "Bar\nBaz"}});
    assertThat(m.toString()).isEqualTo("""
        +-------+-------+
        | Hello | World |
        +-------+-------+
  ...
  public void testNestedToString() {
    Matrix[][] data = new Matrix[][] {
        {new Matrix<>(new Complex[][] {{new Complex(1, 2), new Complex(3, 4)},
            {new Complex(5, 6), new Complex(7, 8)}}),
            new Matrix<>(new Complex[][] {{new Complex(9, 10), new Complex(11, 12)},
                {new Complex(13, 14), new Complex(15, 16)}})},
        {new Matrix<>(new Complex[][] {{new Complex(17, 18), new Complex(19, 20)},
            {new Complex(21, 22), new Complex(23, 24)}}),
            new Matrix<>(new Complex[][] {{new Complex(25, 26), new Complex(27, 28)},
    ...
    assertThat(mat.toString()).isEqualTo("""
        ...
        +---------------------------------+---------------------------------+
        | +--------------+--------------+ | +--------------+--------------+ |
        | | (17.0, 18.0) | (19.0, 20.0) | | | (25.0, 26.0) | (27.0, 28.0) | |
        | +--------------+--------------+ | +--------------+--------------+ |
        | | (21.0, 22.0) | (23.0, 24.0) | | | (29.0, 30.0) | (31.0, 32.0) | |
        | +--------------+--------------+ | +--------------+--------------+ |
        +---------------------------------+---------------------------------+
            """);
  }
  ...
  @Test
  public void testDot() {
    Matrix<Complex> a = new Matrix<>(new Complex[][] {{new Complex(1, 2), new Complex(3, 4)},
        {new Complex(5, 6), new Complex(7, 8)}});

    Matrix<Complex> b = new Matrix<>(new Complex[][] {{new Complex(9, 10), new Complex(11, 12)},
        {new Complex(13, 14), new Complex(15, 16)}});

    Matrix<Complex> c = a.dot(b, Maja::mul, Maja::add);

    assertThat(c)
        .isEqualTo(new Matrix<>(new Complex[][] {{new Complex(-28, 122), new Complex(-32, 142)},
  ...
  public void testEq() {
    ...
    Matrix<Complex> c = new Matrix<>(new Complex[][] {{new Complex(1, 2), new Complex(3, 4)},
        {new Complex(5, 6), new Complex(7, 8)}});
  ...
  public void testRavel() {
    ...
    List<Complex> expectedRavel =
        of(new Complex(1, 2), new Complex(3, 4), new Complex(5, 6), new Complex(7, 8));
  ...
  public void testTranspose() {
    ...
    Matrix<Complex> expected = new Matrix<>(new Complex[][] {{new Complex(1, 2), new Complex(5, 6)},
  ...
  public void testSum() {
    ...
    DoubleMatrix expected = new DoubleMatrix(new double[][] {{6, 8}, {10, 12}});
  ...
  public void testDet() {
    ...
    DoubleMatrix b = new DoubleMatrix(new double[][] {{1, 2, 3}, {4, 5, 6}, {7, 8, 9}});

    assertThat(b.det()).isEqualTo(0);

    DoubleMatrix c =
        new DoubleMatrix(new double[][] {{1, 2, 33, 4}, {5, 6, 7, 8}, {12, 0, 1, 2}, {3, 4, 5, 6}});

    assertThat(c.det()).isEqualTo(-1560);

    DoubleMatrix d = new DoubleMatrix(new double[][] {{1, 2, 33, 4, 5}, {5, 6, 7, 8, 9},
        {12, 0, 1, 2, 3}, {3, 44, 5, 6, 7}, {1, 2, 3, 4, 5}});
  ...
  public void testInverse() {
    ...
    DoubleMatrix expected = new DoubleMatrix(new double[][] {{-2, 1}, {1.5, -0.5}});

    assertThat(a.invert()).isEqualTo(expected);

    // 3x3
    DoubleMatrix b = new DoubleMatrix(new double[][] {{1, 2, 3}, {0, 1, 4}, {5, 6, 0}});

    DoubleMatrix expected2 =
        new DoubleMatrix(new double[][] {{-24, 18, 5}, {20, -15, -4}, {-5, 4, 1}});

    assertThat(b.invert()).isEqualTo(expected2);

    // 4x4
    DoubleMatrix c =
        new DoubleMatrix(new double[][] {{1, 2, 3, 4}, {0, 1, 4, 5}, {6, 7, 0, 8}, {9, 0, 1, 2}});
    ...
    // 5x5
    DoubleMatrix d = new DoubleMatrix(new double[][] {{1, 2, 3, 4, 5}, {0, 1, 4, 5, 6},
        {7, 8, 0, 9, 10}, {11, 12, 13, 0, 14}, {15, 16, 17, 18, 0}});

    DoubleMatrix expected4 = new DoubleMatrix(new double[][] {
        {-3.6879999999999984, 2.143999999999999, 0.36533333333333334, 0.13733333333333325,
            0.041333333333333264},
        {3.933333333333332, -2.4666666666666655, -0.32592592592592595, -0.11481481481481473,
            -0.025925925925925852},
        {-0.16266666666666674, 0.18133333333333335, -0.0708148148148148, 0.030962962962962956,
            0.021185185185185182},
        {-0.26933333333333304, 0.23466666666666647, 0.05214814814814813, -0.041629629629629634,
            0.02414814814814814},
        {-0.3226666666666666, 0.26133333333333325, 0.058074074074074084, 0.03318518518518518,
            -0.02992592592592593}});

    assertThat(d.invert()).isEqualTo(expected4);

    // 1x1
    DoubleMatrix e = new DoubleMatrix(new double[][] {{1}});

    DoubleMatrix expected5 = new DoubleMatrix(new double[][] {{1}});
  ...
  public void testEigenvalues() {
    ...
    Complex[] expected =
        new Complex[] {new Complex(-0.3722813232690143, 0), new Complex(5.372281323269014, 0)};

    assertThat(a.eigen().e()).isEqualTo(expected);

    DoubleMatrix b = new DoubleMatrix(new double[][] {{1, 2, 3}, {4, 5, 6}, {7, 8, 9}});

    Complex[] expected2 = new Complex[] {new Complex(16.116843969807057, 0),
        new Complex(-1.1168439698070447, 0), new Complex(-8.046297179356069E-16, 0) // slightly
                                                                                    // above machep.
                                                                                    // should be 0.
    };
  ...
  @Test
  public void testSVD() {
    DoubleSVDResult a = new DoubleMatrix(new double[][] {{1, 2}, {3, 4}}).svd();
